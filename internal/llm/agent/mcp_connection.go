...
func (c *defaultMCPConnection) Call(ctx context.Context, callID, toolName, input string) (tools.ToolResponse, error) {
	...
	if c.wire != nil && c.wire.Enabled() { c.wire.Out(c.name, toolName, callID, input) }
	start := time.Now()
	result, err := c.cl.CallTool(ctx, mcp.CallToolRequest{Params: mcp.CallToolParams{Name: toolName, Arguments: args}})
	dur := time.Since(start)
	if err != nil {
		if c.wire != nil && c.wire.Enabled() { c.wire.Err(c.name, toolName, callID, dur, err) }
		return tools.NewTextErrorResponse(err.Error()), nil
	}
	var output strings.Builder
	for _, v := range result.Content {
		if v, ok := v.(mcp.TextContent); ok { output.WriteString(v.Text) } else { fmt.Fprintf(&output, "%v: ", v) }
	}
	resp := tools.NewTextResponse(output.String())
	if result.IsError { resp.IsError = true }
	if c.wire != nil && c.wire.Enabled() { c.wire.In(c.name, toolName, callID, resp, dur) }
	return resp, nil
}
